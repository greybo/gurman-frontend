...
import {
  ...
  Smartphone, ArrowRight, Download, User,
} from 'lucide-react';
...
export default function AppUpdateTracking() {
  ...
  const toggleDevice = (deviceModel) => {
    setExpandedDevice(expandedDevice === deviceModel ? null : deviceModel);
  };
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder="Пристрій, версія, працівник..."
        ...
        <div className="flex items-center gap-2 px-3 py-2 bg-gray-50 rounded-lg text-sm text-gray-600">
          ...
          {searchQuery && <span>/ {totalDevices}</span>}
          <span>пристроїв</span>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
        {loading ? (
          ...
          <table className="w-full">
            ...
            <tbody>
              {devices.map((device) => {
                const isExpanded = expandedDevice === device.deviceModel;
                const lastUpdate = device.updates[device.updates.length - 1];
                return (
                  <React.Fragment key={device.deviceModel}>
                    <tr
                      onClick={() => toggleDevice(device.deviceModel)}

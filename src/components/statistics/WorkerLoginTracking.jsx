...
import { Search, ChevronDown, ChevronRight, Calendar, Loader2, Smartphone, LogIn } from 'lucide-react';
import useWorkerLoginData from '../../hooks/useWorkerLoginData';
...
export default function WorkerLoginTracking() {
  const {
    selectedDate,
    setSelectedDate,
    loading,
      ...
      <div className="flex flex-wrap gap-3 items-end">
        ...
        <div className="flex-1 min-w-[200px]">
          ...
          <input
            ...
            placeholder="Пошук по імені працівника..."
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-gray-50 border-b border-gray-200">
                ...
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Входів</th>
                <th className="text-left px-4 py-3 text-xs font-semibold text-gray-500 uppercase tracking-wider">Останній вхід</th>
            ...
            <tbody>
              {workers.map((worker) => {
                ...
                const lastLogin = worker.logins[worker.logins.length - 1];
                return (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span className="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium bg-brand-50 text-brand-700">
                          {worker.logins.length}
                        </span>
                      </td>
                      <td className="px-4 py-3 text-sm text-gray-600 font-mono">{formatTime(lastLogin.timestamp)}</td>
                      <td className="px-4 py-3 text-sm text-gray-600">{lastLogin.deviceModel}</td>
                    </tr>
